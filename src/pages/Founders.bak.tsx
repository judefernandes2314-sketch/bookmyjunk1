...
const foundersStorageKey = "bmj_founders_data";
...
const Founders = () => {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(foundersStorageKey);
      if (!raw) return;
      const parsed = JSON.parse(raw);
      if (!Array.isArray(parsed)) return;
      const normalized = parsed.filter(isFounder);
  ...
  const founderCards = useMemo(() => founders, [founders]);
