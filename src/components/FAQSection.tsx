...
const FAQSection = () => {
  const [cmsFaqs, setCmsFaqs] = useState<Array<{ question: string; answer: string }>>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${base}/api/cms/faq`, { method: "GET" });
        if (!res.ok) return;
        const data = (await res.json()) as unknown;
        if (Array.isArray(data) && data.length > 0) {
          setCmsFaqs(data as any);
        }
      } catch {
  ...
  const faqsToShow = useMemo(() => {
    if (Array.isArray(cmsFaqs) && cmsFaqs.length > 0) {
      const mapped = cmsFaqs
        .filter((x) => typeof x?.question === "string" && x.question.trim() !== "" && typeof x?.answer === "string")
