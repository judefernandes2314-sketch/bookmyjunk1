...
const Footer = () => {
  ...
  const facebookUrl = (cmsFooter?.facebook_url || "").trim() || "https://www.facebook.com/bookmyjunkindia/";
  const instagramUrl = (cmsFooter?.instagram_url || "").trim() || "https://www.instagram.com/bookmyjunkofficial/?hl=en";
  const youtubeUrl = (cmsFooter?.youtube_url || "").trim() || "https://www.youtube.com/@bookmyjunke-wasterecycling8439";
  const linkedinUrl = (cmsFooter?.linkedin_url || "").trim() || "https://in.linkedin.com/company/bookmyjunk";

  const copyrightText =
    (cmsFooter?.copyright_text || "").trim() ||
    `© ${new Date().getFullYear()} BookMyJunk.com — Certified E-waste Recycling Company in India`;

  // Safely parse services — DB returns JSON string or array
  const safeServices = useMemo((): string[] => {
    const raw = (cmsFooter as any)?.services;
    if (!raw) return ["Doorstep E-waste Collection","Electronic Waste Pickup Service","Corporate IT Asset Disposition","Data Destruction Services","Recycle Old Laptop Service","TV Recycling Pickup Service"];
    if (Array.isArray(raw)) return raw;
    try { const parsed = JSON.parse(raw); return Array.isArray(parsed) ? parsed : []; } catch { return []; }
  }, [cmsFooter]);

  // Safely parse quick_links — DB returns JSON string or array
  const safeQuickLinks = useMemo((): { label: string; url: string }[] => {
    const raw = (cmsFooter as any)?.quick_links;
    if (!raw) return [
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            ...
            <ul className="space-y-3 text-base text-black/60">
              {safeServices.map((svc: string, i: number) => <li key={i}>{svc}</li>)}
            </ul>
          </motion.div>
          ...
          <motion.div variants={itemVariants}>
            ...
            <ul className="space-y-3 text-base text-black/60">
              {safeQuickLinks.map((lnk: {label:string;url:string}, i: number) => (
                <li key={i}>
